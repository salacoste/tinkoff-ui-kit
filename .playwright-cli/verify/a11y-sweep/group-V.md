...
| Check | Evidence |
|---|---|
| 1 Keyboard | Unit matrix: FIRST row anchor is the single tab stop `data-table.test.ts:203`; arrows CLAMPED no-wrap `:213`; Home/End `:247`; Enter NATIVE (anchor navigates), Space = click `:265`; inert rows skipped `:325`; roving survives data change `:362`; focusin follows real focus `:448`; non-anchor keydown inert `:459`. LIVE: `tests/visual/data-table.spec.ts:59` (chromium keyboard contract). ENGINE: sweep V/walk playground — EXACT 1 kit stop, ring resolved on the ROW carrier (the new `ringAncestor: '.row'` topology), ×2 themes. |
| 2 Ring | The WHOLE ROW rings — `.row__link:focus-visible { outline: none }` + `.row:has(.row__link:focus-visible)` `data-table.css.ts:187-194`, keyboard-only guard (module header `:34-39`); structural pin `data-table.test.ts:431`; the engine now pins it LIVE (the ancestor carrier the v1 engine could not see — closed deliberately per-target). |
...
| F2 | Kit-wide `:host([hidden])` sweep (deferred-work 6.3 N6). | EXECUTED: 18 unguarded sheets gained the guard; the PATTERN test `tests/hidden-guard.test.ts` found FIVE more — the multi-sheet components (combobox-search, cookie-banner, filter-chips, modal, tooltip) carried the guard only in their SURFACE sheets, leaving the kit HOSTS unguarded (`hidden` on tk-modal rendered!). 33/33 sheets now guarded; pattern-level pins (guard-in-SAME-sheet + roster tripwire 33/27). |
