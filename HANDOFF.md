...
| **v2** | 🚧 6.2+6.3+7.1+6.4+7.2 ИСПОЛНЕНЫ (6.2: tk-filter-chips + tk-pagination; 6.3: tk-combobox-search; 7.1: MegaNav — расширение tk-navbar, Spec Change Log прецедент; 6.4: tk-data-table e06e844 — типографическая row-as-link таблица, APG-клавиатурный слой, §9-строка delta-on-hover; 7.2: tk-cookie-banner — worktree 773f53d → merge 0f1592d, gate-фиксы 9086551, спека закрыта). Итоги окна: 833 unit + visual 1145/1145 (merged main), 24 компонента/враппера; пруфы — `.playwright-cli/verify/{filter-chips,pagination,combobox-search,mega-nav,data-table,cookie-banner}/`. ДОЛГ ЗАКРЫТ в окне 7.2: axe re-entrancy race вылечен (`tests/visual/axe-serialize.ts` chain+busy-retry — co-driver это SB a11y-addon в preview; 4-е вхождение заставило). В РАБОТЕ ПАРАЛЛЕЛЬНО: 6.5 (stocks-catalog showcase, executor на main) + 7.3 (stepper/store-badges/qr-block, executor в worktree). ПРАВИЛО: порт 6007 машинно-глобален — два `test:visual` параллельно НЕ запускать (см. deferred-work) |
