...
| **v2** | 🚧 6.2+6.3+7.1+6.4+7.2+6.5 ИСПОЛНЕНЫ (6.2: tk-filter-chips + tk-pagination; 6.3: tk-combobox-search; 7.1: MegaNav; 6.4: tk-data-table — e06e844 + §9-фикс 13bedc2 + lens-фикс eb9fdd7 (мёртвый селектор `.cell__link`; эмпирически пиксельно-нейтрален — бейзлайны md5-идентичны), спека закрыта; 7.2: tk-cookie-banner; 6.5: stocks-catalog showcase-композиция b1a1b9f — 5 поверхностей живо проводкой, 39-шаговый записанный keyboard-walkthrough, a11y-леджер group-IV, lens SHIP 0/0/2, спека закрыта). Итоги окна: 833 unit + visual 1154/1154 ×2 (merged main, приватный порт 6031), 24 компонента/враппера; пруфы — `.playwright-cli/verify/{filter-chips,pagination,combobox-search,mega-nav,data-table,cookie-banner,stocks-catalog}/`. ДОЛГ axe re-entrancy ЗАКРЫТ (`tests/visual/axe-serialize.ts` chain+busy-retry; co-driver = SB a11y-addon). В РАБОТЕ: 7.3 (stepper/store-badges/qr-block, executor в worktree); 7.4+7.5 КОМПОЗИРУЮТ его трио — стартуют только после его мержа. ПРАВИЛА ПОРТА (заострено): 6007 машинно-глобален, два `test:visual` параллельно НЕ запускать; И — `reuseExistingServer` однажды молча гонял suite против ЧУЖОГО устаревшего dist (сирота с cwd=дерево хватал 6007 за ~40с дважды) — для критичных gate-кругов приватный порт через временный VISUAL_PORT-конфиг |
