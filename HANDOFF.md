...
| **v2** | 🚧 6.2 ИСПОЛНЕНА (tk-filter-chips + tk-pagination; 21 компонентов/обёрток; пиксель-пруфы и девиации — `.playwright-cli/verify/{filter-chips,pagination}/`, спек закрыт). 6.1 до неё (c999e12: токен-слой). Далее: 6.3 (ComboboxSearch) |
...
3. Стори 6.1 по циклу §5 — ✅ исполнена 2026-09-24 (spec-6-1 закрыт, c999e12;
   ...
   транскрипта, триаж/фиксы/верификация довершены оркестратором). Следующая: 6.3
   (ComboboxSearch, спек уже восстановлен в implementation-artifacts).
