...
const meta: Meta<NavbarArgs> = {
  ...
  argTypes: {
    ...
    subActiveValue: {
      control: 'text',
      description:
        'Активная секция строки 2 (subLinks): вес 700 + чернильный цвет + серый штрих 2px. Значение вне subLinks ничего не помечает (выбор матрицы); навигация, не переключатель.',
    },
    subLabel: {
      control: 'text',
      description:
        'Имя второго nav-ориентира (по умолчанию «Разделы»; пустая строка возвращается к умолчанию — фикс 8.1).',
    },
    sticky: { control: 'boolean', description: 'Липкая шапка (по умолчанию включена).' },
    burgerLabel: {
      control: 'text',
      description:
        'Имя кнопки-бургера и диалога меню (пустая строка возвращается к умолчанию «Меню» — фикс 8.1).',
    },
  },
