  ...
  render: () => html`
    ...
    <main class="tks-canvas">
      ...
      <table>
        ...
        <tbody>
          <tr>
            <td>Tab на триггер</td>
            <td>«Выберите повышенный кэшбэк, комбинированный список, Категория» — роль + выбранное</td>
          </tr>
          <tr>
            <td>Enter / ArrowDown (открытие)</td>
            <td>«развернуто», визуальный фокус: «Категория, 1 из 4»</td>
          </tr>
          <tr>
            <td>Стрелки</td>
            <td>опции перечисляются по одной: имя + позиция; disabled-строки пропускаются</td>
          </tr>
          <tr>
            <td>Enter (выбор)</td>
            <td>«Категория выбрана», меню свёрнуто, фокус на триггере</td>
          </tr>
          <tr>
            <td>Esc</td>
            <td>закрытие без изменения значения</td>
          </tr>
        </tbody>
      </table>
    </main>
...
export const Api: Story = {
  name: 'API',
  render: () => apiReferenceDoc('tk-select'),
};
