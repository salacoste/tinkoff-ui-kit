...
| 2026-09-24 | tk-data-table deltas on row hover (Story 6.4) | The delta text (`delta-positive`/`delta-negative`) also renders on the row-HOVER composite surface at 4.039:1 (light) — below the 4.5:1 AA text floor — where the 6.1 token constraint had pinned delta text to surface-base cells only. | Direction-by-COLOR is the table's core semantic (FR-12: «color carries direction»); suppressing or re-coloring deltas on hovered rows would break the semantic mid-interaction, and the no-new-tokens rule forbids inventing delta-on-tint variants. Hover is transient; resting rows sit on surface-base at full compliance. The composite ratio is PINNED in contrast.test.ts (4.039:1 on the rgba(36,74,127,0.06) hover over surface-base); the dark side rides the 6.1 dark first-pass and is verified by the 8.2 sweep. Revisit if the token layer ever gains delta-on-tint variants. |
