...
export const Page: Story = {
  ...
  render: () => html`
    ...
    <div class="tkv2">
      ...
      <p class="tkv2-note">
        ...
        осевой линии, заголовок, необязательный подзаголовок
        <code>slot="subtitle"</code> и текст по центру. Нумерация
        автоматическая 1..n; числа в данных нет и быть не может.
      </p>
  ...
  <p slot="subtitle">Если у вас не зарегистрирован бизнес, сначала оставьте заявку на регистрацию — поможем бесплатно</p><!-- необязательный подзаголовок -->
  ...
  document.querySelector('tk-stepper').steps = [
    ...
    // номер НЕ задаётся: нумерация автоматическая 1..n
      ...
      <div class="tkv2-gotcha">
        ...
        только когда слот непуст (эталон CTA не несёт). Необязательный
        слот <code>subtitle</code> (10.2) — подзаголовок МЕЖДУ заголовком
        и карточками: обёртка тоже рендерится только когда слот непуст
        (пустой слот не добавляет ритма). Пустые
